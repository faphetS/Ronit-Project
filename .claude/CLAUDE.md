...
- **What's done:** Webhook ingest at `POST /api/meta/webhook` with HMAC-SHA256 signature verification (timing-safe). GET handshake at same path echoes `hub.challenge`. Dev-only `POST /api/meta/test-inject` for end-to-end testing. Incoming messages are classified via OpenRouter and routed to Monday.com CRM. **Auto-reply templates (2026-07 copy update — short, link-free except where noted).** Routing in `pickReplyTemplate` (`meta.outbound.service.ts`) is (service) × (phone?) × (first contact vs. answer to the service question):
  - **⚠️ TWO-TRIP UMAN FLOW (2026-09-16).** The client sells two Uman flights in parallel — `kislev` (ר"ח כסלו 11-15/11/26) and `hanukkah` (חנוכה 6-10/12/26) — so the uman path now asks WHICH before replying. `pending_clarifications` carries a second axis (`stage` = `service`|`trip`, plus `trip`), and `detectTrip()` ([Server/src/lib/trip.ts](Server/src/lib/trip.ts)) resolves her answer by keyword, NOT the LLM (the classifier only knows `uman|challah`). Detection accepts names (`חנוכה` / `כסלו` / `ר"ח כסלו`) and dates (`נובמבר` / `דצמבר` / `11-15` / `6-10`); **hanukkah is tested first** because that trip also falls in the month of Kislev, and bare `כסלו` means the Rosh Chodesh trip per the client's own flyer caption. Ordinals ("הראשון") return null — never guessed.
  - **uman opener** → `IG_MSG_UMAN_TRIP_ASK` (the two-date question; no flyer — she hasn't chosen). Her answer → `IG_MSG_UMAN_{KISLEV,HANUKKAH}_PHONE_{PRESENT,MISSING}` + that trip's flyer. A trip named up front ("מעוניינת בחנוכה") skips the question entirely and is treated as uman even when the classifier returns `service: null`. Trip re-asks are capped at 3 (`MAX_REASKS`, same as the service axis) with a **fresh counter** via `advanceToTripStage()`; at the cap the pending row is KEPT, so a later clear trip word is still answered.
  - **challah opener** → `IG_MSG_SERVICE_PHONE_PRESENT` / `IG_MSG_SERVICE_PHONE_MISSING`. Challah is untouched by the two-trip work — trips are uman-only.
  - **vague opener** (interested, no service named) → `IG_MSG_ASK_SERVICE` ("challah or uman?"), re-asked up to 3× via `pending_clarification`; answering `חלה` routes to `IG_MSG_CHALLAH_ANSWER_PHONE_PRESENT` / `IG_MSG_CHALLAH_ANSWER_PHONE_MISSING`, answering `אומן` advances to the trip stage above. `IG_MSG_PHONE_*` and `IG_MSG_UMAN_ANSWER_PHONE_*` are **DEPRECATED** — no longer read by the DM path, kept defined in `env.ts` only because the VPS still sets them (deleting the schema entries would be a silent deploy trap). Strip those 4 lines from the VPS `.env` on deploy.
  ...
  - **Seasonal trip pitch + flyers (two trips live, revisit Kislev after 2026-11-15 and Hanukkah after 2026-12-10):** the trip copy now lives in **code defaults**, not VPS overrides — `IG_MSG_UMAN_TRIP_ASK` + the 4 `IG_MSG_UMAN_{KISLEV,HANUKKAH}_PHONE_*` templates in `env.ts` carry the client's verbatim Hebrew. This deliberately reverses the old override-pile pattern: the repo is now authoritative, a copy change is a PR, and nothing has to be remembered and unwound on the VPS. Each trip reply is followed by a second bubble — that trip's flyer, an `image` attachment at `https://api.ronitbarash.site/static/uman-kislev.jpeg` / `uman-hanukkah.jpeg` (served via `express.static` from [Server/public/](Server/public/), `COPY public ./public` in the Dockerfile). Per-trip map in `meta.outbound.service.ts` (`TRIP_FLYERS`); `file.jpg` is no longer referenced anywhere. Best-effort: text-send failure skips the flyer; flyer failure retries once (~1s) then logs, never throws. NOT sent for: challah, ask-service, phone-thanks, the trip QUESTION itself, comment private replies (Meta hard limit — **one private reply per comment, ever**; second bubble impossible there), WhatsApp/website paths. To add or swap a trip: drop the named original under `Server/public/`, add its `TRIP_FLYERS` entry + templates + `detectTrip()` keywords, commit, then **`git pull && docker compose up -d --build crm`** — `public/` is baked into the image, so a plain restart ships the old image and the flyer 404s silently. History: hilula 27-30/9 (2026-08-04 → 2026-09-14), Kislev-only 11-15/11 (2026-09-14 → 2026-09-16), Kislev + Hanukkah (2026-09-16 →). ⚠️ The code defaults for `IG_MSG_UMAN_ANSWER_PHONE_MISSING` / `IG_MSG_COMMENT_UMAN` still carry the stale `{form_link}` teaser killed 2026-09-01 — the first is now deprecated and unreachable, the second is still live if the אומן comment flow is ever re-enabled.
  - **Known product trade-off of the two-trip flow:** a woman who writes "אומן" used to get details + flyer + phone ask in ONE reply, so her details landed even if she never wrote again. She now gets a question first, and details/phone ask arrive only if she answers — 3 re-asks soften this but do not remove it. Accepted deliberately (two trips cannot both be pitched in one bubble, and the old single bubble mis-sold Hanukkah leads on Kislev copy). The lever that would remove the downside is putting the phone ask into `IG_MSG_UMAN_TRIP_ASK` as well — a client decision, not taken as of 2026-09-16.
  - **Comment → private-reply DM flows** (`meta.comment.service.ts`, gated by `IG_COMMENT_HANDLER_ENABLED`, paced via `ig_comment_queue` + shared cap `IG_COMMENT_REPLY_MAX_PER_HOUR`, cron drain every 1 min, ≤5/tick, 6-day queue expiry, one private reply per comment ever). Two keywords, `kind` column routes the drain:
    - **אומן (any post) — 🛑 OFF since 2026-09-01 per Shahar** (`IG_COMMENT_UMAN_ENABLED=false` on VPS; gate added in `0e84366`, gates ingest AND drops already-queued uman rows at drain). When ON: `IG_MSG_COMMENT_UMAN` DM + Monday Uman lead + known-sender registration; skips existing live leads. Reason for shutdown: the DM's teaser + `{form_link}` pointed at orhazadik.online, which still pitched the departed ט"ו באב trip (IG renders the link as a preview card). Before re-enabling: the site needs updating AND note the VPS also overrides `IG_MSG_COMMENT_UMAN` to a short no-link phone-ask (2026-09-01) while the code default still carries the stale teaser + link.
...
- **Env in use:** `META_APP_SECRET`, `META_VERIFY_TOKEN`, `IG_ACCESS_TOKEN`. IG message templates (all defaulted in `env.ts`, overridable per var): the 9 `IG_MSG_*` templates listed above + `IG_MSG_COMMENT_UMAN` / `IG_MSG_COMMENT_KNIFE`; comment flow: `IG_COMMENT_HANDLER_ENABLED`, `IG_COMMENT_UMAN_ENABLED` (false on VPS since 2026-09-01), `IG_COMMENT_REPLY_MAX_PER_HOUR`, `IG_COMMENT_KNIFE_MEDIA_ID`.
