...
- **What's done:** Webhook ingest at `POST /api/meta/webhook` with HMAC-SHA256 signature verification (timing-safe). GET handshake at same path echoes `hub.challenge`. Dev-only `POST /api/meta/test-inject` for end-to-end testing. Incoming messages are classified via OpenRouter and routed to Monday.com CRM. **Auto-reply templates (2026-07 copy update — short, link-free except where noted).** Routing in `pickReplyTemplate` (`meta.outbound.service.ts`) is (service) × (phone?) × (first contact vs. answer to the service question):
  ...
  - **Seasonal uman trip pitch (text + flyer — current trip: ר"ח כסלו 11-15/11/2026, set 2026-09-14; revisit after 2026-11-15):** the 4 uman DM templates (`UMAN_PHONE_*` + `UMAN_ANSWER_PHONE_*`) are overridden on the VPS `.env` with the trip pitch (no-phone variants = Or's text verbatim, phone-present variants swap the last line for "העברנו את הפנייה למשרד…"; backup `.env.bak-2026-09-14`) and are followed by a second bubble: the trip flyer image, sent as an `image` attachment pointing at `https://api.ronitbarash.site/static/file.jpg` (served via `express.static` from [Server/public/](Server/public/), `COPY public ./public` in the Dockerfile). Hardcoded in `meta.outbound.service.ts` (`FLYER_IMAGE_URL` + `FLYER_TEMPLATE_LABELS`) — deliberate user decision, no env var; removal = code change. Best-effort: text-send failure skips the flyer; flyer failure retries once (~1s) then logs, never throws. NOT sent for: challah, ask-service, phone-thanks, comment private replies (Meta hard limit — **one private reply per comment, ever**; second bubble impossible there), WhatsApp/website paths. To swap next season's flyer: add the named original under `Server/public/` (e.g. `uman-kislev.jpeg`), copy it over `file.jpg` (the name the code serves), commit + rebuild. History: hilula 27-30/9 (2026-08-04 → 2026-09-14), Kislev 11-15/11 (2026-09-14 →). ⚠️ When a trip expires do NOT just delete the 4 overrides — the code defaults for `IG_MSG_UMAN_ANSWER_PHONE_MISSING` / `IG_MSG_COMMENT_UMAN` still carry the stale `{form_link}` teaser killed 2026-09-01.
